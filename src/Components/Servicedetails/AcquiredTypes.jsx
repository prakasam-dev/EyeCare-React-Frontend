...
const AcquiredTypes = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}data/cataract-service-details.json`)
      .then((res) => res.json())
      .then((json) => {
        const acquired = json.categories.find(
          (cat) => cat.type === "Acquired Cataracts"
        );
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        ...
        <header className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Learn about the different types of cataracts that develop later in
            life due to age, trauma, or systemic conditions.
          </p>
        </header>
        ...
        <div className="grid gap-10 md:grid-cols-2">
          {conditions.map((type, index) => (
            ...
            >
              ...
              <p className="text-gray-700">
                <strong>Symptoms:</strong> {type.symptoms.join(", ")}
              </p>
              <p className="text-gray-700">
                <strong>Signs:</strong> {type.signs.join(", ")}
              </p>
              <p className="text-gray-700">
                <strong>Risk Factors:</strong> {type.riskFactors.join(", ")}
              </p>
              <p className="text-gray-700">
                <strong>Treatment:</strong> {type.treatment}
              </p>
              <p className="text-gray-700">
                <strong>Complications (With Surgery):</strong>{" "}
                {type.complications.withSurgery.join(", ")}
              </p>
              <p className="text-gray-700">
                <strong>Complications (Without Surgery):</strong>{" "}
                {type.complications.withoutSurgery.join(", ")}
              </p>
            </div>
          ))}
        </div>
