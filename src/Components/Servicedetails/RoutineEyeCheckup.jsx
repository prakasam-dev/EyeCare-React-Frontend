...
const RoutineEyeCheckup = () => {
  ...
  useEffect(() => {
    fetch('/data/routine-eye-checkup-tests.json')
  ...
  if (!checkupData) {
    return <p className="text-center py-10 text-pink-600 font-medium">Loading routine eye checkup details...</p>;
  }

  const { checkupTitle, checkupPurpose, tests } = checkupData;

  return (
    <section id="routine-eye-checkup" className="py-20 bg-gradient-to-br from-pink-100 to-purple-100">
      <div className="max-w-screen-xl mx-auto px-4">
        {/* Header */}
        <header className="text-center mb-16" data-aos="fade-up">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">{checkupTitle}</h2>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">{checkupPurpose}</p>
        </header>
    ...
    loading="lazy"/>
              ...
              <div className="lg:w-1/2 w-full space-y-3">
               <h3 className="text-2xl sm:text-2xl md:text-3xl font-bold text-pink-600 text-center">
                ...
                <p className="text-gray-700 text-lg"><strong>Why it’s done:</strong> {test.whyItIsDone}</p>
                {test.investigation && (
                  <p className="text-gray-700 text-lg"><strong>Investigation:</strong> {test.investigation}</p>
                )}
                {test.procedure && (
                  <p className="text-gray-700 text-lg "><strong>Procedure:</strong> {test.procedure}</p>
                )}
                <div className="pt-4 flex justify-center text-lg">
                  <span className={`px-4 py-2 text-sm font-semibold rounded-full
                    ${test.importanceLevel === 'High' ? 'bg-red-100 text-red-700' :
                      test.importanceLevel === 'Medium' ? 'bg-yellow-100 text-yellow-700' :
        ...
        <footer className="mt-24 text-center" data-aos="fade-up">
          <h3 className="text-2xl font-bold text-gray-800 mb-4">Want to experience all these?</h3>
