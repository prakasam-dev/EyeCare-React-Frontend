...
<header className='bg-gradient-to-r from-pink-100 to-purple-100 sticky top-0 z-50'>
  <div className='max-w-screen-xl mx-auto px-4 py-4 flex justify-between items-center relative'>
    {/* Logo and nav */}
    <Link to='/' className='flex items-center focus:outline-none focus:ring-2 focus:ring-pink-600 rounded'>
      <FaEye size={40} className='gap-10px text-pink-600 p-1 m-1'/>
      <h1 className='text-2xl font-bold text-pink-600'>DIVINE VISION CENTRE</h1>
    </Link>

    <nav className='hidden md:flex space-x-6' aria-label='Primary navigation'>
        {navLinks.map((link,index)=>(
            <Link key={index}
            to={link.to} className='text-gray-700 hover:text-pink-600 transition-colors duration-200
              focus:outline-none focus:ring-pink-600 focus:rounded'>{link.label}</Link>
        ))}
    </nav>
    ...
    <nav className='hidden md:block' aria-label='Appointment booking'>
      <a href='#appointment'
        ...
        focus:ring-pink-800 focus:ring-offset-2'>Schedule your visit</a>
    </nav>
  </div> 
          ...
          {navLinks.map((link,index)=>(
            <li key={index}>
              <Link to={link.to}
                ...
                onClick={toggleMenu}>{link.label}</Link>
            </li>
          ))}
        </ul>
      </nav>
