...
const Hero = () => {
  ...
  return (
    <section id='home' className='relative overflow-hidden bg-gradient-to-r from-pink-100 to-purple-100'>
      <article className='max-w-screen-xl mx-auto px-4 lg:px-8 sm:px-6 py-14 md:py-20 flex flex-col-reverse lg:flex-row 
      items-center gap-10 lg:gap-20'>
        ...
        <hgroup data-aos='fade-up' data-aos-delay='500'>
            <h1 className='text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-800 mb-4'>
                When eye heal, the world<mark className='text-pink-600 bg-transparent'> become light</mark>
            </h1>
            <p className='text-base sm:text-lg text-gray-600 mb-8 max-w-lg mx-auto lg:mx-0'>
            Comprehensive diagnostics and treatment for all eye conditions with world-class specialists</p>
        </hgroup>
        ...
        lg:justify-start gap-4 mb-8'>
            ...
            aria-label='Book a consultation'>Schedule your visit</Link>
            <Link to='/Services'
            className='border border-pink-600 text-pink-600 hover:bg-pink-50 px-8 py-3 rounded-full
            text-center transition'
            aria-label='Explore our Services'>Explore Services</Link>
        </nav>
            ...
            <ul className='flex flex-wrap justify-center gap-4 md:gap-8 text-center '>
                {status.map((status,index)=>(
                    <li key={index} className='px-2'>
                        <strong className='text-2xl font-bold text-pink-600'>{status.value}</strong>
                        <small className='block text-sm text-gray-600'>{status.label}</small>
                    </li>
                ))}
            </ul>
        </aside>
        ...
        <div className='relative w-full max-w-md'>
            ...
            <img src={Profile} alt='eye check profile'
